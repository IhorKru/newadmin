...
class adkService extends PublisherController
{
    public function adkServiceAction($numcampaigns, $timezone, $depdate)
    {
        ...
        try {
            if (file_exists('../web/working/adk/adkresult.xml')) unlink('../web/working/adk/adkresult.xml');
            ...
            foreach ($batcharray as $sizecnt) {
                ...
                foreach ($xmlarrayB as $xmlarrayBA) {
                    ...
                    file_put_contents('../web/working/adk/adkresult.xml', $result, FILE_APPEND);
                }
                unset($xmlarray);
                unset($xmlarrayB);
                ## PARSING XML RESPONCE
                # prepare our stream to be read with a 16mb buffer
                $errorstream = new Stream\File('../web/working/adk/adkresult.xml', 1024);
                ...
                $curbatch = $querybatch->getSingleScalarResult();
                # prepare our stream to be read with a 16mb buffer
                $emailstream = new Stream\File('../web/working/adk/adkresult.xml', 1024);
                ...
                unlink('../web/working/adk/adkresult.xml');
            }
        } catch (Exception $e) {
            unset($subscribers);
            unset($subscribersB);
